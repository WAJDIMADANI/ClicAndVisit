...
object KeyStoreHelper {
    ...
    internal fun createKeysJBMR2(context: Context, alias: String) {
        ...
        val spec = KeyPairGeneratorSpec.Builder(context)
            // You'll use the alias later to retrieve the key1. It's a key1
            // for the key1!
            .setAlias(alias)
        ...
        DebugLog.d(TAG, "Public Key is: " + kp.public.toString())
    ...
    internal fun createKeysM(alias: String, requireAuth: Boolean) {
        try {
            ...
            DebugLog.d(TAG, "Public Key is: " + keyPair.public.toString())
    ...
    fun isSigningKey(alias: String): Boolean {
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
            try {
                ...
                DebugLog.e(TAG, e.message.toString(), e)
                false
            }
    ...
    private fun getPrivateKeyEntry(alias: String): KeyStore.PrivateKeyEntry? {
        try {
            ...
            if (entry == null) {
                DebugLog.w(TAG, "No key1 found under alias: $alias")
                DebugLog.w(TAG, "Exiting signData()...")
                return null
            }

            if (entry !is KeyStore.PrivateKeyEntry) {
                DebugLog.w(TAG, "Not an instance of a PrivateKeyEntry")
                DebugLog.w(TAG, "Exiting signData()...")
                return null
            }
            return entry
